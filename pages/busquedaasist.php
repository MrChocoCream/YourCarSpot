            ...
            <div class="flex flex-col gap-8" style="justify-content: center; align-items: center; width: 100vw; padding-top: 20px;">
                ...
                if (isset($_SESSION['error_message'])) {
                    ...
                    <div style="margin-top: 40px;;">
                        <label for="consumo" class="block text-sm font-medium text-gray-700 textSZ">¿Qué tan bueno quieres que sea el consumo te interesa?</label>
                        <select id="consumo" onchange="updateConsumo()" name="consumo" placeholder="consumo">
                                        <option value="" selected>Consumo</option>
                                        <?php
                                        if ($consumo) {
                                            if ($consumo->num_rows > 0) {
                                                while ($datos = $consumo->fetch_assoc()) {
                                                    echo "<option value=\"{$datos['idconsumo']}\">{$datos['descripTipoconsumo']}</option>";
                                                }
                                            }
                                            $consumo->free();
                                        } else {
